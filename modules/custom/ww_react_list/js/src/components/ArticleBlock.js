...
class ArticleBlock extends React.Component {
   ...
   */
  componentDidMount() {
    fetch('/jsonapi/node/timeline_entry?sort=-created&page[limit]=30')
        ...
        <ul>
          {data.map(article =>
            <li key={article.id}>
              ...
              <a href="#" styleName={styles.link} className={styles.link} onClick={this.handleClick.bind(this, article)}>{article.attributes.title}</a>
            </li>
          )}
        </ul>
      </div>
